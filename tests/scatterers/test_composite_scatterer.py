...
@pytest.fixture
def composite_example_interact_path1(composite_example_methods):
    return composite_example_methods['interact_path1']

@pytest.fixture
def composite_example_scatter(composite_example_methods):
    return composite_example_methods['scatter']

@pytest.mark.skipif(not test.USE_CUDASIM, reason='no CUDASIM')
def test_cudasim_interact_path1(composite_example_interact_path1):
...
@pytest.mark.skipif(not test.USE_CUDASIM, reason='no CUDASIM')
def test_cudasim_scatter(composite_example_scatter):
    neutron = np.array([0.0,0,0, 0,0,1000, 0,0, 0, 1.])
    threadindex = 0
    rng_states = None
    N = 10 #000
    ...
    for i in range(N):
        neutron = np.array([0.0,0,0, 0,0,1000, 0,0, 0, 1.])
        composite_example_scatter(threadindex, rng_states, neutron)
        print(neutron)
        p1 = neutron[-1]
        if p1>0:
            p+=p1
...
def test_cuda_interact_path1(composite_example_interact_path1):
    @cuda.jit
...
@pytest.mark.skipif(not test.USE_CUDA, reason='no CUDA')
def test_cuda_scatter(composite_example_scatter):
    @cuda.jit
    def scatter_kernel(rng_states, neutrons, n_neutrons_per_thread):
        N = len(neutrons)
        thread_index = cuda.grid(1)
        start_index = thread_index*n_neutrons_per_thread
        end_index = min(start_index+n_neutrons_per_thread, N)
        for i in range(start_index, end_index):
            composite_example_scatter(thread_index, rng_states, neutrons[i])
    def run(neutrons):
        N = len(neutrons)
        threads_per_block = 128
        ntotthreads = int(1e5)
        ...
        from mcvine.acc.config import rng_seed
        rng_states = create_xoroshiro128p_states(actual_nthreads, seed=rng_seed)
        scatter_kernel[nblocks, threads_per_block](rng_states, neutrons, n_neutrons_per_thread)
    N = int(10)
    neutrons = np.zeros((N, 10))
    neutron = np.array([0.0,0,0, 0,0,1000, 0,0, 0, 1.])
    neutrons[:] = neutron
