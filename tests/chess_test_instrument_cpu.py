...
import math
import mcvine
import mcvine.components as mccomps
...
COMPONENT guide_gravity1 = Guide_gravity(
    w1=0.03, 
    h1=0.03, 
    w2=0.03, 
    h2=0.03, 
...
COMPONENT Hchopper = MultiDiskChopper(
 slit_center = locdeg, slit_width = angledeg,
 nslits = nslitsRRM, delta_y = 0.32,
 nu = 15, jitter = 6E-05,
 delay = fhToF, radius = 0.35,
 abs_out = 1)
AT (0,0,29.3) RELATIVE Origin

COMPONENT guide_gravity2 = Guide_gravity(
    w1=0.03, 
    h1=0.03, 
    w2=0.03, 
    h2=0.03, 
    l=0.67)
AT (0, 0, 29.31) RELATIVE Origin

COMPONENT Mchopper1 = DiskChopper(
 theta_0 = 8, radius = 0.35,
 yheight = 0.045, nu = -300,
 nslit = 1, jitter = 1E-06,
 delay = fm1ToF)
AT (0,0,30.0-0.025/2) RELATIVE Origin

COMPONENT Mchopper2 = DiskChopper(
 theta_0 = 8, radius = 0.35,
 yheight = 0.045, nu = 300,
 nslit = 1, jitter = 1E-06,
 delay = fm2ToF)
AT (0,0,30.0+0.025/2) RELATIVE Origin
ROTATED (180,0,0) RELATIVE Origin


COMPONENT Lsample = L_monitor(
 nL = 1000, filename = "L_Sample.dat",
 xwidth = 0.1, yheight = 0.1,
 Lmin = 0, Lmax = 20,
 restore_neutron = 1)
AT (0,0,30.1) RELATIVE Origin

COMPONENT BeamStop = Beamstop(
 radius = 0.25)
AT (0,0,33.0) RELATIVE Origin
...
def instrument():
    ...
    source = mccomps.sources.Source_simple(
        'src',
        radius=0.015, 
        dist=0.751, 
        xw=0.03, 
    ...
    Ltmp = mccomps.monitors.L_monitor(name="tmpmon",
                                 filename = "L_Guide.dat",
                                 xwidth=0.1, yheight=0.1,
                                 Lmin=0, Lmax=20,
                                 nchan=1000)
    ...
    chopper1 = mccomps.optics.DiskChopper_v2(name="Mchopper1", theta_0=8, 
                           radius = 0.35,
                           yheight=0.045, nu=-300,
                           nslit=1, jitter=1E-06,
                           delay=fm1ToF)
    ...
    chopper2 = mccomps.optics.DiskChopper_v2(name="Mchopper2", theta_0=8, 
                           radius=0.35,
                           yheight=0.045, nu=300,
                           nslit=1, jitter=1E-06,
                           delay=fm2ToF)
    ...
    Lsample = mccomps.monitors.L_monitor(name="mon",
